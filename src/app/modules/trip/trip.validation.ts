...
const createTripSchema = z
  ...
  .refine(
    (data) => data.fromCountry.toLowerCase() !== data.toCountry.toLowerCase(),
    {
      message: "From country and to country cannot be the same",
      path: ["toCountry"],
    },
  );
  ...
  .refine(
    (data) => {
      if (data.fromCountry && data.toCountry) {
        return data.fromCountry.toLowerCase() !== data.toCountry.toLowerCase();
      }
      return true;
    },
    {
