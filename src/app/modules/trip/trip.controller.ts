...
const getAvailableShipments = catchAsync(async (req: Request, res: Response) => {
  const result = await TripService.getAvailableShipments(
    req.query as Record<string, unknown>,
    req.user!,
  );
  ...
  sendResponse(res, {
    statusCode: 200,
    success: true,
    message: "Available shipments fetched successfully",
    meta: result.meta,
    data: result.data,
  });
});
