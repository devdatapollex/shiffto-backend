...
const defaultCategories = [
  { name: "Electronics", slug: "electronics", minPrice: 10, maxPrice: 100, maxWeight: 50, maxQuantity: 10 },
  { name: "Clothing", slug: "clothing", minPrice: 5, maxPrice: 50, maxWeight: 100, maxQuantity: 50 },
  { name: "Documents", slug: "documents", minPrice: 2, maxPrice: 20, maxWeight: 2, maxQuantity: 5 },
  { name: "Food Items", slug: "food-items", minPrice: 4, maxPrice: 30, maxWeight: 30, maxQuantity: 20 },
];
...
async function seedShipments() {
  ...
  if (stepDefinitions.length === 0) {
    console.log("No step definitions found. Seeding default step definitions...");
    const defaultSteps = [
      { stage: shipmentStepStage.PAYMENT_CONFIRMED, label: "Payment confirmed", order: 1, description: null },
      { stage: shipmentStepStage.PICKED_UP, label: "Picked up", order: 2, description: null },
      { stage: shipmentStepStage.CHECKED_IN, label: "Checked in", order: 3, description: null },
      { stage: shipmentStepStage.IN_TRANSIT, label: "In transit", order: 4, description: "Flight is on the way to destination" },
      { stage: shipmentStepStage.ARRIVED_AT_DESTINATION, label: "Arrived at destination", order: 5, description: null },
      { stage: shipmentStepStage.OUT_FOR_DELIVERY, label: "Out for delivery", order: 6, description: null },
      { stage: shipmentStepStage.DELIVERED, label: "Delivered", order: 7, description: null },
    ];
  ...
  for (const item of shipmentsToSeed) {
    ...
    console.log(`Successfully seeded shipment: ${item.itemName} (${item.fromCountry} -> ${item.toCountry})`);
  }
