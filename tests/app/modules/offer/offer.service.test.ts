...
import { OfferStatus, ShipmentStatus } from "../../../../src/generated/prisma/enums";
...
describe("OfferService", () => {
  ...
  describe("acceptOffer", () => {
    const mockOfferData = {
      ...
      shipment: {
        ...
        itemName: "iPhone 15",
        shipmentSteps: [
          { id: "step-1", stage: "PAYMENT_CONFIRMED", order: 1, completedAt: null },
    ...
    it("should accept offer and deduct capacity inside transaction", async () => {
      ...
      mockOffer.update.mockResolvedValue({ ...mockOfferData, status: OfferStatus.ACCEPTED });
      mockOffer.findUnique.mockResolvedValueOnce({ ...mockOfferData, status: OfferStatus.ACCEPTED });
      mockShipmentStepService.confirmPayment.mockResolvedValue([]);

      const { OfferService } = await importService();
      const result = await OfferService.acceptOffer("offer-1", mockSenderUser as any);
      expect(result).toBeDefined();
      ...
      expect(mockShipmentStepService.confirmPayment).toHaveBeenCalledWith("shipment-1", mockSenderUser, mockPrisma);
    });
  ...
  describe("rejectOffer", () => {
    ...
    it("should reject offer and update status", async () => {
      mockOffer.findUnique.mockResolvedValue(mockOfferData);
      mockOffer.update.mockResolvedValue({ ...mockOfferData, status: OfferStatus.REJECTED });

      const { OfferService } = await importService();
      const result = await OfferService.rejectOffer("offer-1", mockSenderUser as any);
      expect(result).toBeDefined();
